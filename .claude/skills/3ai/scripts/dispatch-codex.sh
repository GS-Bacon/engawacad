...
# stdin prefix 生成: SCOPE PROFILE → SCOPE DEFENSE (extra-input) の順で連結
build_prefix() {
  local prefix=""
  ...
  if [[ -n "$EXTRA_INPUT_FILE" && -f "$EXTRA_INPUT_FILE" ]]; then
    local content
    content="$(cat "$EXTRA_INPUT_FILE")"
    prefix+="===== SCOPE DEFENSE =====$(printf '\n')以下は本 Issue のスコープ外。指摘・拡張提案・改善要求の対象としないこと。$(printf '\n')${content}$(printf '\n')===== END SCOPE DEFENSE =====$(printf '\n\n')"
  fi
  printf '%s' "$prefix"
}
