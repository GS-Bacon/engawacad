...
fn raw_mesh_signed_volume(mesh: &mycad_kernel::tessellation::TriangleMesh) -> f64 {
    let mut vol = 0.0_f64;
...
fn t01_determinism() {
    ...
    let get_seam_circle = |solid: &mycad_kernel::brep::topology::Solid,
                           face: &mycad_kernel::brep::topology::Face|
     -> (
        mycad_kernel::geometry::Point,
        mycad_kernel::geometry::Vec3,
        f64,
    ) {
...
fn t06_topology() {
    ...
    match &edge.curve {
        Curve::Circle { normal, .. } => {
            let neg_y = mycad_kernel::geometry::Vec3::new(0.0, -1.0, 0.0);
            assert!(
