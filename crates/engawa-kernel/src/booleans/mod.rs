...
mod tests {
    ...
    fn make_planar_solid_with_points(points: Vec<Point>, id_gen: &mut IdGenerator) -> Solid {
        ...
        let v_indices: Vec<usize> = points
            .into_iter()
            .map(|p| {
                let name = engawa_format::EntityRef::try_named(
                    "test",
                    engawa_format::EntityKind::Vertex,
                    "v",
                )
                .ok();
        ...
        for i in 0..v_indices.len() {
            ...
            let e_idx = solid.add_edge(
                ...
                [0.0, 1.0],
                engawa_format::EntityRef::try_named("test", engawa_format::EntityKind::Edge, "e")
                    .ok(),
            );
        ...
        let name =
            engawa_format::EntityRef::try_named("test", engawa_format::EntityKind::Face, "f").ok();
    ...
    fn t16d_tiny_analytic_circle_rejected() {
        ...
        let e = solid.add_edge(
            ...
            engawa_format::EntityRef::try_named("test", engawa_format::EntityKind::Edge, "e").ok(),
        );
