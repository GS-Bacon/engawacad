...
use engawa_kernel::geometry::{curve::Curve, surface::Surface, Point, Vec3};
use engawa_kernel::primitives::{make_cuboid, make_sphere};
use engawa_kernel::tessellation::{tessellate_solid, TessellationError, TriangleMesh};
...
fn t03_arbitrary_axis_circ_normal() {
    ...
    let opts = engawa_kernel::tessellation::TessellationOptions {
        angular_segments: 16,
        ..Default::default()
    };

    let result = engawa_kernel::tessellation::tessellate_sphere_face_trimmed(
        &solid,
        &solid.faces[face],
        face,
        &opts,
