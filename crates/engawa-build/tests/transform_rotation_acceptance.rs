...
fn t06_boundary_seam_cylinder_axis_rotation() {
    ...
    let cyl_axis = solid.faces.iter().find_map(|f| {
        if let engawa_kernel::geometry::surface::Surface::Cylinder { axis, .. } = &f.surface {
            Some(*axis)
        } else {
            None
        }
    ...
    use engawa_kernel::geometry::math::orthonormal_basis;
