...
fn t05_cut_now_supported() {
    ...
    assert!(
        matches!(
            result,
            Err(engawa_kernel::error::KernelError::EmptyBooleanResult)
        ),
...
fn t05_fuse_disjoint_boxes() {
    ...
    assert!(
        matches!(
            result,
            Err(engawa_kernel::error::KernelError::DisjointFuseResult)
        ),
...
fn t09_intersect_contact_only() {
    ...
    assert!(
        matches!(
            err,
            engawa_kernel::error::KernelError::EmptyBooleanResult
                | engawa_kernel::error::KernelError::DegenerateBooleanContact
        ),
...
fn t17_boolean_stl_export() {
    use engawa_format::Feature;
    use engawa_kernel::tessellation::{tessellate_solid, to_ascii_stl};
...
fn t21_nonplanar_input_cone_rejected() {
    ...
    if let Err(e) = &result {
        assert!(
            !matches!(
                e,
                engawa_kernel::error::KernelError::NonPlanarBooleanInput { .. }
            ),
...
fn t18_intersection_edge_names() {
    ...
    for (i, name) in named_edges.iter().enumerate() {
        match name {
            engawa_format::EntityRef::Derived { op, .. } => {
                assert!(
                    op.contains("cut"),
...
fn build_a3_input() -> Vec<engawa_format::Feature> {
    use engawa_format::Feature;
    vec![
        Feature::CreateBox {
            id: "box".into(),
...
fn a3_box_cut_contained_sphere() {
    ...
    for shell in &solid.shells {
        if shell.faces.len() == 1 {
            ...
            if matches!(
                face.surface,
                engawa_kernel::geometry::surface::Surface::Sphere { .. }
            ) {
                assert!(
                    !face.same_sense,
...
fn build_a1_input() -> Vec<engawa_format::Feature> {
    use engawa_format::Feature;
    vec![
        Feature::CreateBox {
            id: "box1".into(),
...
fn a1_mesh_volume_abs(mesh: &engawa_kernel::tessellation::TriangleMesh) -> f64 {
    let mut vol = 0.0_f64;
