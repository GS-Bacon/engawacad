#!/usr/bin/env bats
# guard-crates.ts のユニットテスト

load helpers/common

GUARD="$SCRIPTS_DIR/guard-crates.ts"
...
@test "TC6a: crates/** のパスは block される (exit=2)" {
  INPUT='{"tool_name":"Edit","tool_input":{"file_path":"crates/engawa-kernel/src/lib.rs"}}'
...
@test "TC6c: CAD_WORKER=1 のとき crates/** も通過する (exit=0)" {
  INPUT='{"tool_name":"Edit","tool_input":{"file_path":"crates/engawa-kernel/src/lib.rs"}}'
  run bash -c "export CAD_WORKER=1; echo '$INPUT' | bun '$GUARD'"
  [ "$status" -eq 0 ]
}
