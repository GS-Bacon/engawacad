#!/usr/bin/env bats
# state.ts のユニットテスト（loops.glm_impl のインクリメント）

load helpers/common

STATE_SH="$SCRIPTS_DIR/state.ts"
...
@test "TC5: state.sh inc loops.glm_impl が 0→1→2 と単調増加し JSON が壊れない" {
  bun "$STATE_SH" init "$STATE_FILE" 99 test-slug
...
"
  ...
  # 1 回インクリメント
  run bun "$STATE_SH" inc "$STATE_FILE" glm_impl
  run python3 -c "
import json
d = json.load(open('$STATE_FILE'))
...
"
  ...
  # 2 回目インクリメント
  bun "$STATE_SH" inc "$STATE_FILE" glm_impl
  run python3 -c "
import json
d = json.load(open('$STATE_FILE'))
print(d['loops']['glm_impl'])
